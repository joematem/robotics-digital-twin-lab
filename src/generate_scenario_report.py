...
def interpret(kpis):
    ...
    for row in kpis.itertuples(index=False):
        ...
        if scenario == "baseline":
            lines.append(
                "- Baseline shows normal fleet operation and provides the reference condition."
            )
        elif "blocked_zone" in scenario:
            lines.append(
                f"- {scenario} introduced {row.delayed_records} delayed records, showing route disruption and local congestion effects."
            )
        elif "charging" in scenario:
            lines.append(
                f"- {scenario} introduced {row.waiting_for_charger_records} waiting-for-charger records, showing support-infrastructure stress."
            )
        else:
            lines.append(
                f"- {scenario} produced {row.total_records} telemetry records and should be interpreted against the baseline."
            )
...
- These scenarios can support digital twin research into resilience, disruption response, and operational stress.
